...
// run a loop:
    ...
    // check if move is valid
                    ...
                    while (!valid) 
                    {
                        ...
                        if (inBounds(row, column)  
                            ...
                            && adjacencyCheckX(row, column, rowO, columnO))
                        {
                           valid = true;
    ...
    // Check if someone won or  if there is a tie
        ...
        if (i == 49)
        {
            printf("Do you want to place the last block (i.e. take your last turn)(2 to exit out, anything else for no):?\n");
            fgets(tempBuffer, BUFFER_SIZE, stdin);
            int inOUT = atoi(tempBuffer);
    ...
    printf("Player 2's turn:\n");
                ...
                // check if move is valid
                    ...
                    while (!valid) 
                    {
                        ...
                        printf("CHECK 1: %d\n", adjacencyCheckO(row, column, rowX, columnX));
...
bool adjacencyCheckX(int row, int column, int rowO, int columnO){
      ...
      if((row == rowO - 1 && column == columnO - 1) 
      || (row == rowO - 1 && column == columnO)           
      || (row == rowO - 1 && column == columnO + 1) 
...
bool adjacencyCheckO(int row, int column, int rowX, int columnX){
      ...
      if((row == rowX - 1 && column == columnX - 1) 
      || (row == rowX - 1 && column == columnX)            
      || (row == rowX - 1 && column == columnX + 1) 
